...
OPEN_H<tuple<int, int, int, int, int>> open_h;
...
struct Waiting_Node {
    ...
    void data(int id, int c, int h) {
        ...
        if (id >= (int)H.size()) {
            cnt.resize(id + 1);
            H.resize(id + 1);
            certain_min.resize(id + 1, INF);
        }
        cnt[id] = c;
        H[id] = h;
        certain_min[id] = h;
    }
    void change_certain_min(int par_id, int h) {
        unique_lock<mutex> lk(mtx);
        certain_min[par_id] = min(certain_min[par_id], h);
    }
    vector<tuple<pair<int, int>, int, int>> push(const int par_id, const int id,
                                                 int h, int hash_distribution) {
        unique_lock<mutex> lk(mtx);
        waiting_nodes[par_id].emplace_back(id, h, hash_distribution);
        vector<tuple<pair<int, int>, int, int>> nodes;
        auto it = waiting_nodes.find(par_id);
        if (it != waiting_nodes.end() &&
            (int)it->second.size() == cnt[par_id]) {
            int mn = certain_min[par_id];
            for (auto [cid, ch, c_hash_distribution] : waiting_nodes[par_id]) {
                if (mn > ch) mn = ch;
                nodes.push_back(
                    make_tuple(make_pair(ch, 1), cid, c_hash_distribution));
            }
            waiting_nodes.erase(it);
#ifdef PUHF4
            for (auto& [ch, p, c_hash_distribution] : nodes) {
                if (ch.first == mn) ch.second = 0;
            }
#endif
        }
        return nodes;
...
void Evaluate(const int thread_id, const int par_id, const int state_id,
    ...
    auto v = waiting_node[par_hash_distribution].push(par_id, state_id, h,
                                                      hash_distibution);
    if (v.size()) {  // all siblings of now_state have been evaluated
        waiting_node[par_hash_distribution].mtx.lock();
        int par_h = waiting_node[par_hash_distribution].H[par_id];
        waiting_node[par_hash_distribution].mtx.unlock();
        open_f.mtx.lock();
        for (auto [ch, id, c_distribution] : v) {
            if (ch.first == INF) continue;
            if (open_inserted_states[c_distribution].inserted(id))
                continue;  // if have been inserted open, continue
            open_f.push(ch.first, ch.second, {id, c_distribution}, thread_id);
        }
        auto it = open_f.mn.find(par_h);
        open_f.mn.erase(it);
        open_f.mtx.unlock();
    }
}
...
void Search(const int thread_id) {
    while (!flg()) {
        ...
        if (open_h.size()) {
            ...
            open_h.mtx.unlock();
            open_f.mtx.lock();
            auto [H, now_state_id_and_hashdistibution] = open_f.pop(
                thread_id);  // if h(top(open)) <= min(h(expanding_state)) or
                             // top(open) is certain pop top(open),
                             // else H.first <0
            int now_h = H.first;
...
#ifdef PUHF3
                if (now_h == open_f.min()) {
                    open_f.move(now_h);  // set all nodes in open which have
                                         // h_value = now_h certain
                }
#endif
            }
            open_f.mtx.unlock();
