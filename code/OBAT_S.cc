...
struct OPEN_Arrays : OPEN {
    ...
    pair<int, T> pop(const int thread_id = -1) {
        if (aborted) return {-2, T()};  // no solutaion
        if (thread_id != -1) {
            if (cnt == 0) {
                if (mn.size() == 1) {
                    aborted = true;
...
// LIFO
template <typename T>
struct OPEN_Arrays_LIFO : OPEN {
    vector<vector<T>> Heap;
    int best;
    int cnt;
    multiset<int> mn;
    OPEN_Arrays_LIFO() : best(INF), cnt(0) {
        mn.insert(INF);
    }
    ...
    pair<int, T> _pop() {
        while (Heap[best].empty()) ++best;
        pair<int, T> data = make_pair(best, Heap[best].back());
        Heap[best].pop_back();
        --cnt;
        return data;
    }
...
// Random
template <typename T>
struct OPEN_Arrays_Random : OPEN {
    vector<vector<T>> Heap;
    int best;
    int cnt;
    multiset<int> mn;
    OPEN_Arrays_Random() : best(INF), cnt(0) {
        mn.insert(INF);
    }
    ...
    pair<int, T> _pop() {
        while (Heap[best].empty()) ++best;
        int k = rand() % Heap[best].size();
        pair<int, T> data = make_pair(best, Heap[best][k]);
        Heap[best][k] = Heap[best].back();
        Heap[best].pop_back();
        --cnt;
        return data;
    }
...
struct GENERATED {
    unordered_map<int, bool> gen;
    mutex mtx;
    int is_generated(int hash) {
        unique_lock<mutex> lk(mtx);
        return gen[hash];
    }
    bool open_in(int hash) {
        unique_lock<mutex> lk(mtx);
        if (gen[hash]) return false;
        gen[hash] = 1;
        return true;
    }
};
...
OPEN_OBAT open;
UNEVALUATED<tuple<int, int, int, int, int>> unevaluated;
...
struct Waiting_Node {
    ...
    // probress?,min,{h,{id,hash_distribution}}
    tuple<bool, int, vector<pair<int, pair<int, int>>>> push(
        const int par_id, const int id, int h, int hash_distribution) {
        unique_lock<mutex> lk(mtx);
        waiting_nodes[par_id].emplace_back(id, h, hash_distribution);
        vector<pair<int, pair<int, int>>> nodes;
        auto it = waiting_nodes.find(par_id);
        bool possibility_progress_node = false;
        int par_h = -1;
        if (it != waiting_nodes.end() &&
            (int)it->second.size() == cnt[par_id]) {
            par_h = H[par_id];
            for (auto [cid, ch, c_hash_distribution] : waiting_nodes[par_id]) {
                if (ch == INF) continue;
                if (par_h > ch) {
                    possibility_progress_node = true;
                }
                nodes.push_back({ch, {cid, c_hash_distribution}});
            }
            waiting_nodes.erase(it);
        }
        return {possibility_progress_node, par_h, nodes};
    }
};
...
void Expand(const int thread_id, int now_h, int state_id,
    ...
    vector<tuple<int, int, int, int, int>> succesor_states;
    for (auto i : operatable_id) {
        ...
        int next_state_id = close_f[next_hash_distribution].id(next_hash);
        if (next_state_id == -1) {
            next_state_id = close_f[next_hash_distribution].states.size();
...
void Evaluate(const int thread_id, const int par_id, const int state_id,
    ...
    int h =
        h_values[hash_distibution].h_val(hash);  // if s have been
                                                 // evaluated, using the cash
    if (h == -1) {
#ifdef FF
        h = H_FF(s.state, goal_pair, goal, thread_id);
#endif
        h_values[hash_distibution].add(hash, h);
        ++evaluated_states;
    }
    auto [possibilty_progress_node, par_h, v] =
        waiting_node[par_hash_distribution].push(par_id, state_id, h,
                                                 hash_distibution);
    if (par_h != -1) {  // all siblings of s have been evaluated
        open.mtx.lock();
        if (possibilty_progress_node) {  // parent(s) is a b-state
            open.Deffered_push(par_h, {par_id, par_hash_distribution},
                               v);  // insert parent(s) into Deffered
        } else {                    // parent(s) is an a-state
            for (auto [ch, c_id_and_distribution] :
                 v) {  // insert(siblings of s into open)
                open.open_n_push(ch, c_id_and_distribution);
            }
        }
        auto it = open.open_n.mn.find(par_h);
        open.open_n.mn.erase(it);
        open.mtx.unlock();
    }
}
...
void Search(const int thread_id) {
    while (!flg()) {
        ...
        if (unevaluated.size()) {
            ...
            if (open_n_tp >=
                Deffered_tp) {  // if h(top(open))>=h(top(Deffered))
                if (Deffered_tp <=
                    expanded_min) {  // if h(top(Deffered))<=
                                     // min(h(expanding_state)) pop
                                     // top(Deffered), else now_h<0
                    open.mv(thread_id);
                }
                open.mtx.unlock();
                continue;
            ...
            if (now_h >= 0) {
                open.open_n.mn.insert(
                    now_h);  // insert now_state into expanding_state
            }
            open.mtx.unlock();
            if (now_h >= 0) {
                Expand(thread_id, now_h, now_state_id, now_hash_distribution);
            }
        }
    }
}
